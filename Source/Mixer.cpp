...
namespace reverb
{
    ...
    {
        wetAudio.multiplyBy(wetRatio);
        
//        wetAudio.addWithMultiply(dryAudioCopy, 1 - wetRatio);
        
        // Create a scaled copy of the dry audio
        AudioBlock scaledDryAudio = dryAudioCopy;
        scaledDryAudio.multiplyBy(1.0f - wetRatio);

        // Add the scaled dry audio to the wet audio
        wetAudio.add(scaledDryAudio);
