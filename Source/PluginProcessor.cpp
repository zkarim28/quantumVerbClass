...
//                                          fnGainTodBString, nullptr );
        // Low-shelf filter cut-off frequency
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(0) + PID_FILTER_FREQ_SUFFIX,  // Parameter ID
                "EQ: Low-shelf cut-off frequency",                               // Parameter name
                "<16-520 Hz>",                                                  // Parameter label with description
                juce::NormalisableRange<float>(16.0f, 520.0f),                  // Range (16 to 520 Hz)
        ...
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(0) + PID_FILTER_Q_SUFFIX,    // Parameter ID
                "EQ: Low-shelf Q factor",                                       // Parameter name
                "<0.71-1.41>",                                                  // Parameter label with description
                juce::NormalisableRange<float>(0.71f, 1.41f),                   // Range (0.71 to 1.41)
                1.06f,                                                          // Default value
                nullptr,                                                        // Value to text function
                nullptr                                                         // Text to value function
            )
        ...
        // Low-shelf filter gain factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(0) + PID_FILTER_GAIN_SUFFIX, // Parameter ID
                "EQ: Low-shelf gain factor",                                    // Parameter name
                "<" + juce::String(gain15dBInv) + " = no change>",              // Parameter label with description
                juce::NormalisableRange<float>(gain24dBneg, gain12dB),          // Range (gain24dBneg to gain12dB)
                gain6dBneg,                                                     // Default value
                fnGainTodBString,                                               // Value to text function
                nullptr                                                         // Text to value function
            )
        );
...
//                                          gain6dBneg,
//                                          fnGainTodBString, nullptr );
        // Peak filter 1 cut-off frequency
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(1) + PID_FILTER_FREQ_SUFFIX, // Parameter ID
                "EQ: Peak filter 1 cut-off frequency",                         // Parameter name
                "<250-1500 Hz>",                                               // Parameter label with description
                juce::NormalisableRange<float>(250.0f, 1500.0f),               // Range (250 to 1500 Hz)
        ...
        // Peak filter 1 Q factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(1) + PID_FILTER_Q_SUFFIX,    // Parameter ID
                "EQ: Peak filter 1 Q factor",                                   // Parameter name
                "<0.71-6.5>",                                                   // Parameter label with description
                juce::NormalisableRange<float>(0.71f, 6.5f),                    // Range (0.71 to 6.5)
                3.38f,                                                          // Default value
                nullptr,                                                        // Value to text function
                nullptr                                                         // Text to value function
            )
        ...
        // Peak filter 1 gain factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(1) + PID_FILTER_GAIN_SUFFIX, // Parameter ID
                "EQ: Peak filter 1 gain factor",                                // Parameter name
                "<" + juce::String(gain15dBInv) + " = no change>",              // Parameter label with description
                juce::NormalisableRange<float>(gain24dBneg, gain12dB),          // Range (gain24dBneg to gain12dB)
                gain6dBneg,                                                     // Default value
                fnGainTodBString,                                               // Value to text function
...
//                                          gain6dBneg,
//                                          fnGainTodBString, nullptr );
        // Peak filter 2 cut-off frequency
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(2) + PID_FILTER_FREQ_SUFFIX, // Parameter ID
                "EQ: Peak filter 2 cut-off frequency",                         // Parameter name
                "<2600-15600 Hz>",                                             // Parameter label with description
                juce::NormalisableRange<float>(2600.0f, 15600.0f),             // Range (2600 to 15600 Hz)
                8000.0f,                                                       // Default value
                nullptr,                                                       // Value to text function
                nullptr                                                        // Text to value function
            )
        );

        // Peak filter 2 Q factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(2) + PID_FILTER_Q_SUFFIX,    // Parameter ID
                "EQ: Peak filter 2 Q factor",                                   // Parameter name
                "<0.71-6.50>",                                                  // Parameter label with description
                juce::NormalisableRange<float>(0.71f, 6.50f),                   // Range (0.71 to 6.50)
                3.38f,                                                          // Default value
                nullptr,                                                        // Value to text function
                nullptr                                                         // Text to value function
            )
        ...
        // Peak filter 2 gain factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(2) + PID_FILTER_GAIN_SUFFIX, // Parameter ID
                "EQ: Peak filter 2 gain factor",                                // Parameter name
                "<" + juce::String(gain15dBInv) + " = no change>",              // Parameter label with description
                juce::NormalisableRange<float>(gain24dBneg, gain12dB),          // Range (gain24dBneg to gain12dB)
                gain6dBneg,                                                     // Default value
                fnGainTodBString,                                               // Value to text function
...
//                                          gain6dBneg,
//                                          fnGainTodBString, nullptr );
        // High-shelf filter cut-off frequency
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(3) + PID_FILTER_FREQ_SUFFIX, // Parameter ID
                "EQ: High-shelf cut-off frequency",                            // Parameter name
                "<8000-21000 Hz>",                                             // Parameter label with description
                juce::NormalisableRange<float>(8000.0f, 21000.0f),             // Range (8000 to 21000 Hz)
        ...
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(3) + PID_FILTER_Q_SUFFIX,    // Parameter ID
                "EQ: High-shelf Q factor",                                      // Parameter name
                "<0.71-1.41>",                                                  // Parameter label with description
                juce::NormalisableRange<float>(0.71f, 1.41f),                   // Range (0.71 to 1.41)
                1.06f,                                                          // Default value
                nullptr,                                                        // Value to text function
        ...
        // High-shelf filter gain factor
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_FILTER_PREFIX + std::to_string(3) + PID_FILTER_GAIN_SUFFIX, // Parameter ID
                "EQ: High-shelf gain factor",                                   // Parameter name
                "<" + juce::String(gain15dBInv) + " = no change>",              // Parameter label with description
                juce::NormalisableRange<float>(gain24dBneg, gain12dB),          // Range (gain24dBneg to gain12dB)
                gain6dBneg,                                                     // Default value
                fnGainTodBString,                                               // Value to text function
...
//                                          fnGainTodBString, nullptr );
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_IR_GAIN,                                        // Parameter ID
                "Impulse response gain",                         // Parameter name
                "<" + juce::String(1.0f) + " = no change>",      // Parameter label with description
                juce::NormalisableRange<float>(0.0f, 1.0f),      // Range (0.0 to 1.0)
                0.5f,                                            // Default value
                fnGainTodBString,                                // Value to text function
                nullptr                                          // Text to value function (not provided)
            )
        );
...
//                                          nullptr, nullptr);
        ...
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_IR_LENGTH,                           // Parameter ID
                "Reverb length",                          // Parameter name
                "Reverb length <s>",                      // Parameter label with description
                juce::NormalisableRange<float>(0.1f, 5.0f), // Range (0.1 to 5.0)
                3.0f,                                     // Default value
                fnGainTodBString,  // Value to text
...
//                                          nullptr, nullptr );
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_PREDELAY,                                      // Parameter ID
                "Pre-delay",                                     // Parameter name
                "<0-1000 ms>",                                   // Parameter label with description
                juce::NormalisableRange<float>(0.0f, 1000.0f),   // Range (0.0 to 1000.0 ms)
                0.0f, nullptr, nullptr                                             // Default value
                // No value-to-text or text-to-value conversion functions
            )
        );
...
//                                          nullptr, nullptr );
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_WETRATIO,                                      // Parameter ID
                "Dry/wet ratio",                                 // Parameter name
                "<0 = dry, 1 = wet>",                            // Parameter label with description
                juce::NormalisableRange<float>(0.0f, 1.0f),      // Range (0.0 to 1.0)
                0.5f, nullptr, nullptr                                             // Default value
                // No value-to-text or text-to-value conversion functions
            )
        );
...
//                                          fnGainTodBString, nullptr );
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_AUDIO_OUT_GAIN,                                  // Parameter ID
                "Output gain",                                   // Parameter name
                "<" + juce::String(gain15dBInv) + " = no change>", // Parameter label with description
                juce::NormalisableRange<float>(gain50dBneg, 1.0f), // Range (gain50dBneg to 1.0)
                1.0f,                                            // Default value
                fnGainTodBString,                                // Value to text function
                nullptr                                          // Text to value function (not provided)
            )
...
//                                          false, true, true );
        parameters.createAndAddParameter(
            std::make_unique<juce::AudioProcessorValueTreeState::Parameter>(
                PID_ACTIVE,                                  // Parameter ID
                "Active",                                    // Parameter name
                "<y/n>",                                     // Parameter label with description
                juce::NormalisableRange<float>(0.0f, 1.0f),  // Range (0.0 to 1.0)
                1.0f,                                        // Default value
                nullptr,                                     // Value to text function
                nullptr,                                     // Text to value function
                false,                                       // Discrete values
                true,                                        // Bypassable
                true                                         // Automatable
            )
        );
